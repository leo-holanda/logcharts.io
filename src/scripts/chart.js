...
Chart.prototype.addNewLine = function(
  id,
  color,
...
Chart.prototype.updateLineByField = function(
  field,
  id,
  xScale = this.xScale.copy(),
  yScale = this.yScale.copy()
...
Chart.prototype.updateMainLine = function(field) {
  ...
  this.chartSVG
    .select('.chart-line')
    ._groups[0][0].setAttribute('field', field);
  ...
  this.chartSVG.select('.y-axis').transition().duration(1000).call(this.yAxis);
  ...
  this.chartSVG
    .select('.chart-line')
    ._groups[0][0].setAttribute('domain', this.xScale.domain());
...
Chart.prototype.addBrush = function() {
  this.brush = d3
    ...
    .extent([
      [this.margin.left, 0],
      [this.width - this.margin.right, this.brushHeight],
    ])
    .on('brush', (event) => {
      /*
...
Chart.prototype.updateByBrush = function(
  selection,
  xScale = this.xScale.copy(),
  yScale = this.yScale.copy()
) {
  ...
  for (selector of document.querySelectorAll('.selector-btn')) {
    ...
    this.chartSVG.select('#' + selector.id).attr(
      'd',
      d3
        .line()
        .defined((row) => fixValue(row[field]) !== undefined)
...
Chart.prototype.addTooltip = function() {
  const tooltip = this.chartSVG.append('g').attr('class', 'tooltip');

  const line = this.chartSVG
    .append('line')
  ...
  const tooltipBackgroundStroke = tooltip
    .append('rect')
  ...
  this.chartSVG.on('touchmove mousemove', function() {
    ...
    const firstField =
      document.querySelector('.selector-div').lastChild.innerHTML;

    const yScale = chart.yScale;
    line.attr(
      'transform',
      'translate(' + xScale(parseTime(row['Time'])) + ',' + 0 + ')'
    );
    tooltip.attr(
      'transform',
      'translate(' +
        xScale(parseTime(row['Time'])) +
        ',' +
        yScale(fixValue(row[firstField])) +
        ')'
    );
    tooltip
      .select('.tooltip-time')
      .text(parseTime(row['Time']).toLocaleTimeString());
  });
