...
export function checkTooltipHorizontalPosition(tooltipParams) {
  ...
  let circleLocation = tooltipParams.tooltipCircle.node().getBoundingClientRect()
  let tooltipBox = circleLocation.left + tooltipParams.tooltipBackground.node().getBoundingClientRect().width - areaOutsideChart

  return tooltipBox > tooltipParams.chart.width ? true : false
}

export function repositionTooltipHorizontally(tooltipParams, isTooltipOutOfScreen) {
...
export function repositionTooltipVertically(tooltipParams, isTooltipOutOfScreen) {
  let circleLocation = tooltipParams.tooltipCircle.node().getBoundingClientRect()
  let offset = (document.querySelectorAll(".selector-btn").length - 1) * 15
  let tooltipVerticalLocation =  circleLocation.bottom + 20 + offset
